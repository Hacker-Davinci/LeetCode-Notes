...
public:
    bool canJump(vector<int>& nums) {
        vector<int> dp(nums.size(), -1);
        int i = 0;
        return solve(dp, nums, i);
    }
    inline bool solve(vector<int> &dp, vector<int> &nums, int i){
        if(i == nums.size()-1) return true; // reach the last index
        if(nums[i] == 0) return false; // we can't jump anymore
        
        if(dp[i] != -1) return dp[i]; // whether can reach or not 
